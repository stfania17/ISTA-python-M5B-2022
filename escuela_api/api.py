...
@app.route('/listado_de_estudiantes')
def listado_de_estudiantes():
    with open('datos\estudiante.csv') as archivo:
        reader = csv.reader(archivo)
        next(reader)
        listado_de_estudiante = []
        for fila in reader:
            listado_de_estudiante.append({'cedula': fila[0],'primer_apellido': fila[1],'segundo_apellido': fila[2],'primer_nombre': fila[3],'segundo_nombre': fila[4]})
    return json.dumps(sorted(listado_de_estudiante, key=lambda a: a['primer_nombre']))
...
def eliminar_asistencia_por_cedula(cedula):
    with open('datos\datos_asistencia.csv', 'r') as file:
        ...
        for columna in reader:
            if columna[0] != cedula:
                listado_de_estudiante.append(columna)
    with open('datos\datos_asistencia.csv', 'w', newline='') as file:
        writer = csv.writer(file, delimiter=',')
        writer.writerows(listado_de_estudiante)
